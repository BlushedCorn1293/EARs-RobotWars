...
const int recieverPin = 13;  // Pin for reading throttle input from RC receiver
...
const int maxThrottle = 7;  // Max throttle as a percentage out of 100
...
void loop() {
  ...
  // if ((switchState != lastSwitchState) || (hallState != lastHallState)) {
    if (switchState == switchOff) {  // If switch is ON (If OFF then will outThrotttle 0)
      ...
      Serial.println("Safety Switch OFF: SHUT DOWN MOTOR");
    } else {
      Serial.println("Safety Switch ON");
      // if (hallState == LOW) {
      // If sensor NOT yet triggered
      recieverOutput = pulseIn(recieverPin, HIGH);  // read pulse output from reciever (Timeout after 20ms if no signal which sets value to 0)
      if (recieverOutput > recieverTriggerValue) {  // If switch on controller has been turned on , max motor speed
        outThrottle = maxThrottle;
        Serial.println("Controller Switch ON: POWER UP MOTOR");
        // }
      }else{
        outThrottle = minThrottle;
      }
